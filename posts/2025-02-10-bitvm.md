...
Mathematically, this is a $(N/2+1)$-of-$N$ trust model.
...
- Every node with indegree $>1$ is either:
  ...
  - a multiplication gate, $times$, that represents the product of its children

Here's an example of an arithmetic circuit that represents the function

$$f(x_1, x_2) = x_1 dot x_2 + x_1$$
...
$x_1$ and $x_2$,
the product gate computes $x_1 dot x_2$,
and the sum gate computes the result of the product gate added to $x_1$.
All of this evaluates to $x_1 dot x_2 + x_1$.
...
- $cal(N P)$ problems, which are **hard to solve, but _easy_ to verify**.
...
$cal(N P)$ stems from non-deterministic polynomial time,
...
once you find an algorithm that solves one of the $cal(N P)$ problems
in polynomial time, you can use it to solve **_any_ $cal(N P)$ problem
in polynomial time**.
But we haven't yet found such algorithms for any $cal(N P)$ problem.
Heck, we don't even know if $cal(P) != cal(N P)$.
...
1. **Soundness**: If the statement is _false_, no cheating prover
   ...
   $$
    forall A, forall x, forall pi: Pr[V(A, pi, x) =
    "accept"] < "negligible".
   $$

   Here $Pr[V(A, pi) = "accept"]$ denotes
...
and $nothing$ to demonstrate the flow of information.
...
1. An empty output $nothing$ that can be spent using either the Payout Optimistic transaction
...
Alice can say "fuck you" and spend the empty output $nothing$ using the Challenge transaction.
