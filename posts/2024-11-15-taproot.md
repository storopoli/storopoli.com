...
since $S_2$ has depth $n = 3$ in the tree it took $ceil(O(log 3)) = 2$
...
hence it took $ceil(O(log 2)) = 1$ step to prove inclusion.
...
This allows us to encode $2^(128) dot 4 "MB"$ of data
