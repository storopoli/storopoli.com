...
In a sense we can say that the function $"omelet"$ is a one-way function

$$"omelet"^(-1)(x) = dots$$

That is, we don't know how to invert the function $"omelet"$ to get the original eggs back.
...
- **Deterministic**: given a function that maps elements from set $X$ to set $Y$, $f: X -> Y$,
  ...
  It is deterministic.
- **Collision resistance**: the possible values of $f: X -> Y$ follows a uniform distribution,
...
called addition $+$ and multiplication $times$**.
We write
...
and $times$ is the multiplication operation.
...
For example $ZZ_5$ is a finite field with 5 elements:

$$ZZ_5 = \{ 0, 1, 2, 3, 4 \}.$$

In general, $ZZ_n$ is a finite field with $n$ elements:
...
The $ZZ_5$ example above is a finite field of order 5.
However, $ZZ_4$ is not a finite field,
because 4 is not a prime number, but rather a composite number.

$$4 = 2 times 2.$$

And we can write $ZZ_4$ as

$$ZZ_4 = 2 times ZZ_2.$$

This means that every element in $a in ZZ_4$ can be written as

$$a = 2 times b,$$

where $b$ is an element in $ZZ_2$.

Hence, not every element of $ZZ_4$ is unique, and they are equivalent to the elements in $ZZ_2$.

In general if $n$ is a composite number,
then $ZZ_n$ is not a finite field.
However, if $n = r times s$ where $r$ and $s$ are prime numbers,
and $r < s$,
then $ZZ_n$ is a finite field of order $r$.
...
For example, suppose that we have numbers $a,b$ in a very large finite field $ZZ_p$,
such that
...
As you've guessed right this is the $ZZ_7$ finite field.
...
Hence, say that $3$ and $5$ are _**generators**_ or _**primitive roots**_ of $ZZ_7$.
...
**You'll need to try each exponent $x in ZZ_p$ until you find the one that satisfies the equation**.
...
1. **Choose a generator $g$ of the group**.
   For $p = 17$, we can choose $g = 3$ because $3$ is a primitive root of $ZZ_(17)$.
...
- $ZZ_p$: the finite field of order $p$
- $g$: a generator of $ZZ_p$
- $S_k$: secret key, a random integer in the finite field $ZZ_p$
...
1. Compute $u_1 = H(m) dot w mod q$.
1. Compute $u_2 = K dot w mod q$.
...
1. $s = k^(-1) dot (H + S_k K) mod q$ ($mod p$ and $H(m)$ implicit).
1. $k = s^(-1) dot (H + S_k K) mod q$ (move $s$ to $k$).
1. $k = H dot s^(-1) + S_k K dot s^(-1) mod q$ (distribute $s^(-1)$).
1. $k = H dot w + S_k K dot w mod q$ ($w = s^(-1)$).
1. $g^k = g^(H dot w + S_k K dot w mod q)$ (put $g$ in both sides).
1. $g^k = g^(H dot w mod q) dot g^(S_k K dot w mod q)$ (product of the exponents).
1. $g^k = g^(H dot w mod q) dot P^(K dot w mod q)_k$ ($P_k = g^(S_k)$).
1. $g^k = g^(u_1) dot P^(u_2)_k$ (replace $u_1$ and $u_2$).
...
Here we are in a context that we have two signatures $s$ and $s'$,
both using the same nonce $k = k'$.
...
Now remember you know $s$, $s'$, $H(m_1)$, $H(m_2)$ $K$, and $K'$.
...
i.e anything with $+, dot, -$, are linear operations
...
can be verified with the public key $P'_k + P_k$.
