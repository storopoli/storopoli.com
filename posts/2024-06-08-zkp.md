...
1. **Completeness**: If the statement is true, the verifier will accept the proof.
   ...
   Here $Pr[V(S, pi) = "accept"]$ denotes the probability that the verifier accepts the proof given a simulator $S$ and a proof $pi$.
...
1. **Soundness**: If the statement is _false_, no cheating prover can convince an honest verifier that it is true,
   ...
   $$ forall A, forall x, forall pi: Pr[V(A, S, pi) = "accept"] < "negligible". $$

   Here $Pr[V(A, S, pi) = "accept"]$ denotes the probability that the verifier accepts the proof given an adversary $A$, a simulator $S$, and a proof $pi$.
...
1. **Zero-Knowledge**: If the statement is true, the verifier learns nothing about the secret $x$.
   ...
   $$ forall x: "View"_V [P(x) <-> V(pi)] = S(x, pi). $$

   Here $"View"_V$ is the view of the verifier $V$,
   and $<->$ denotes the interaction between the prover and the verifier.
...
$$ f(x) = c_d x^d + dots + c_1 x^1 + c_0 x^0 $$

where $c_d, dots, c_1, c_0$ are the coefficients of the polynomial,
...
the decimal approximation is $approx 1.16 times 10^(77)$.
...
This is due to the fact that an adversary has $d/(10^(77))$ chance of guessing the polynomial[^birthday],
which we can safely consider negligible.

[^birthday]: The [Birthday paradox](https://en.wikipedia.org/wiki/Birthday_problem)
  states that any collision resistance scheme has a probability of $1/2$ of collision,
...
$$ (x - a_0) (x - a_1) dots (x - a_d) = 0 $$

where $a_0, dots, a_d$ are the roots of the polynomial.
...
of the factors $t(x) = (x - a_0) dots (x - a_d)$, called the **target polynomial**,
...
1. Verifier samples a random value $r$, calculates $t = t(r)$ and gives $r$ to the
   prover
1. Prover calculates $h(x) = (p(x))/(t(x))$ and evaluates $p = p(r)$ and $h = h(r)$;
   the resulting values $p$, $h$ are provided to the verifier
1. Verifier then checks that $p = t dot h$, if so those polynomials are equal,
...
1. Prover calculates $h(x) = (p(x))/(t(x)) = x$, evaluates $p = p(23) = 10626$ and $h = h(23) = 23$
   and provides $p$, $h$ to the verifier
1. Verifier then checks that $p = t dot h$, i.e. $10626 = 462 dot 23$,
...
- Every node with indegree $>1$ is either:
  ...
  - a multiplication gate, $times$, that represents the product of its parents

Here's an example of an arithmetic circuit that represents the polynomial $p(x_1, x_2) = x_2^3 + x_1 x_2^2 + x_2^2 + x_1 x_2$:

[![Arithmetic Circuit of $p(x_1, x_2)$](https://mermaid.ink/img/pako:eNp1kbFOwzAURX8leitpRKy4Tjyw0JWNCcxg1W4SEduVcUQgykB_gg_iT_gSbJeEthLWG3yP7n13eCNsjZBAYdeZ123DrUvuN0wn_g35I4Ph-_DB4OmXoCM5LMRo6VG-aC5ESF2dAnQGVN85D74-I5mbktXqJobnon8BWoovSDBHFBrOUn8guE8QpFDbVgB1tpcpKGkVDxLGYGfgGqkkA-q_gttnBkxPPrPn-sEYNces6esG6I53L171e8Gd3LS8tlwt1EotpL01vXZAEY47gI4weEVIVhDiJy9JWZRlCm9AMcrwGhUEVQjlFa7WeErhPbZeZyXxG6RonbF3x-vFI04_UByH4A?type=png)](https://mermaid.live/edit#pako:eNp1kbFOwzAURX8leitpRKy4Tjyw0JWNCcxg1W4SEduVcUQgykB_gg_iT_gSbJeEthLWG3yP7n13eCNsjZBAYdeZ123DrUvuN0wn_g35I4Ph-_DB4OmXoCM5LMRo6VG-aC5ESF2dAnQGVN85D74-I5mbktXqJobnon8BWoovSDBHFBrOUn8guE8QpFDbVgB1tpcpKGkVDxLGYGfgGqkkA-q_gttnBkxPPrPn-sEYNces6esG6I53L171e8Gd3LS8tlwt1EotpL01vXZAEY47gI4weEVIVhDiJy9JWZRlCm9AMcrwGhUEVQjlFa7WeErhPbZeZyXxG6RonbF3x-vFI04_UByH4A)

In the circuit above, the input gates compute (from left to right)
$x_1, x_2$ and $1$,
the sum gates compute $x_1 + x_2$
and $x_2 + 1$,
and the product gate computes $(x_1 + x_2) x_2 (x_2 + 1)$
which evaluates to $x_2^3 + x_1 x_2^2 + x_2^2 + x_1 x_2$.
...
[^papers]The whole idea of ZKPs as discussed above in three properties
...
[^papers]: We have tons of papers on the subject.
...
then you should read [@petkus2019zksnarkworks].
