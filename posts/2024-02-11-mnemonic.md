...
$$H(X) = -Sigma_(x in X) P(x_i) log P(x_i) = -(1/2 log 1/2 + 1/2 log 1/2) = 1 "bit"$$
...
$$H(X) = -Sigma_(x in X) P(x_i) log P(x_i) = - sum_(i=1)^6 (1/6 * log 1/6) approx 2.58 "bits"$$

This means that the outcome of a fair 6-sided die has 2.58 bits of Entropy.
we need $op("ceil")(2.58) = 3$ bits to encode the outcome of a fair 6-sided die.
...
$$H = 8 dot log_2(26) approx 37.6 "bits"$$

This means that an attacker would need to try $2^(37.6) approx 2.01 dot 10^(11)$ combinations to guess the password[^birthday-paradox].
...
$$H = 8 dot log_2(95) approx 52.6 "bits"$$

This means that an attacker would need to try $2^(52.6) approx 6.8 dot 10^(15)$ combinations to guess the password.
...
i.e. $3 dot 10^(11)$ hashes/second,
it would take:

1. 8-length lowercase-only password:

$$(2.01 dot 10^(11))/(3 dot 10^(11)) approx 0.67 "seconds"$$

1. 8-length password with uppercase letters, numbers, and symbols:

$$(6.8 dot 10^(15))/(3 dot 10^(11)) approx 22114 "seconds" approx 6.14 "hours"$$
...
If not, check my [post on cryptographics basics](/posts/2024-02-05-crypto-basics.html).
...
you have $11 dot 11 = 121$ bits of Entropy.
And for a 24-word seed phrase,
you have $23 dot 11 = 253$ bits of Entropy.
...
$$(2^(121))/(10^(12)) approx 2.66 dot 10^(24) "seconds" approx 3.08 dot 10^(19) "days" approx 8.43 dot 10^(16) "years"$$

That's a lot of years.
Now for a 253-bit Entropy seed phrase:

$$(2^(253))/(10^(12)) approx 1.45 dot 10^(64) "seconds" approx 1.68 dot 10^(59) "days" approx 4.59 dot 10^(56) "years"$$
...
$$(2^(77))/(10^(12)) approx 1.51 dot 10^(11) "seconds" approx 1.75 dot 10^(6) "days" approx 4.79 dot 10^(3) "years"$$
